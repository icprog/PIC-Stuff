...
int PID_Kp, PID_Ki;
unsigned char PID_Kd;
int PID_Integrated;
int PID_Prev_Input;
int PID_MinOutput, PID_MaxOutput;
unsigned char PID_First_Time;
//int Err;
...
void Init_PID(int Kp, int Ki, char Kd, int minOutput, int maxOutput)
{
  PID_Kp         = Kp;
  PID_Ki         = Ki;
  PID_Kd         = Kd;
...
int PID_Calculate(int const setpoint, int temp)
{
...
//  LCDWriteIntXY(0,0,Err,4);
  ...
  // --- calculate total ---
  Result = ErrValue + PID_Integrated - DerivativeValue; // mind the minus sign!!!
  // limit it to output minimum and maximum
